...
public class TurtleViewTable extends TableView {
	...
	private void setupXposCol(TableColumn<TurtleListInsertion, String> xposCol) {
		...
		xposCol.setOnEditCommit((CellEditEvent<TurtleListInsertion, String> t) -> {
			((TurtleListInsertion) t.getTableView().getItems().get(t.getTablePosition().getRow()))
	...
	private void setupHeadingCol(TableColumn<TurtleListInsertion, String> headingCol) {
		...
		headingCol.setOnEditCommit((CellEditEvent<TurtleListInsertion, String> t) -> {
			((TurtleListInsertion) t.getTableView().getItems().get(t.getTablePosition().getRow()))
					.setHeading(Double.parseDouble(t.getNewValue()));
		});
	}
	...
	private void setupPenThickCol(TableColumn<TurtleListInsertion, String> penThickCol) {
		...
		penThickCol.setOnEditCommit((CellEditEvent<TurtleListInsertion, String> t) -> {
			((TurtleListInsertion) t.getTableView().getItems().get(t.getTablePosition().getRow()))
					.setPenThickness(Double.parseDouble(t.getNewValue()));
		});
	}
	...
	private void setupTableColumns() {
		...
		data = FXCollections.observableArrayList(new TurtleListInsertion(turtles.get(0), true, iteratingID,
				turtles.get(0).getRelativeX(), turtles.get(0).getRelativeY(), turtles.get(0).getOrientation(),
				turtles.get(0).getPenDown(), "Black", 3.0));
		iteratingID += 1;
		table.setItems(data);
	...
	public void updateValues() {
		...
		for (Turtle t : turtles) {
			TurtleListInsertion newInsertion = new TurtleListInsertion(t, t.getActive(), iteratingID, t.getRelativeX(),
					t.getRelativeY(), t.getOrientation(), t.getPenDown(), "Black", t.getPenThickness());
			iteratingID += 1;
			data.add(newInsertion);
			t.updateOnScreen();

			/*
			 * public void updateValues() { List<TurtleListInsertion> insertionList = new
			 * ArrayList<>(); for (Turtle t : turtles) { insertionList.add(new
			 * TurtleListInsertion(t, t.getActive(), t.getID(), t.getX(), t.getY(),
			 * t.getOrientation(), t.getPenDown(), "Black", 0.0));
			 */
		}
